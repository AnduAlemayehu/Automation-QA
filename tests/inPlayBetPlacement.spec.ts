import { test } from './base/loggedInBase';
import { InPlayBettingPage } from '../pages/InPlayBettingPage';
...
for (const data of betStakeData) {
  test(data.testName, async ({ page,homePage }, testInfo) => {
    const inPlay = new InPlayBettingPage(page);

    const MAX_MATCH_TRIES = 2;
    const MAX_STAKE_TRIES = 2;
  ...
  if (page.isClosed()) {
    console.log(`⚠️ Cannot capture screenshot, page already closed (${reason})`);
    return;
  }
      ...
      while (matchIndex < MAX_MATCH_TRIES) {
        ...
        for (let attempt = 1; attempt <= MAX_STAKE_TRIES; attempt++) {
          ...
          if (result === 'SUCCESS') {
            ...
            console.log(
              `💰 Balance diff: ${( balanceAfter).toFixed(2)}`
            );
            console.log(
              `📊 Exposure diff: ${(exposureBefore).toFixed(2)}`
            );
