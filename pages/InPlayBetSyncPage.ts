...
export class InPlayBetSyncPage {
  readonly page: Page;
  ...
  readonly myBetsMenu: Locator;
  readonly myBetsHeader: Locator;

  // My Bets table
  readonly myBetsFirstRow: Locator;
  readonly myBetsStake: Locator;
  readonly myBetsOdds: Locator;
    // Bet Slip selectors (YOUR PROVIDED)
  readonly slipSelection: Locator;
  readonly slipStake: Locator;
  readonly slipOdds: Locator;
  ...
  constructor(page: Page) {
    ...
    this.exposure = page
      ...
      .first();
 // 🔹 My Bets table (first available bet)
    this.myBetsFirstRow = page.locator('tr.bet-item').first();
    this.myBetsStake = this.myBetsFirstRow.locator('td:has-text("10.00")');
    this.myBetsOdds = this.myBetsFirstRow.locator('td').nth(5);
      
    // 🔹 My Bets navigation
    this.userMenuBtn = page.locator(
      "//button[@class='mat-mdc-menu-trigger user-btn']//*[name()='svg']"
    );
    this.myBetsMenu = page.getByText('My Bets', { exact: true });
    this.myBetsHeader = page.locator('span:has-text("My Bets")');

    // 🔹 Bet Slip (exact selectors you gave)
    this.slipSelection = page.locator('span').filter({ hasText: /.+/ }).first();
    this.slipStake = page.locator('span.stake.text-center');
    this.slipOdds = page.locator('span.price.text-center');
